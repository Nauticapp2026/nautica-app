...
export function ModeracionClient({
  ...
  const onDeleted = () => router.refresh();
    ...
    <div className="space-y-5">
      ...
      {tab === 'comunicaciones' &&
        (filteredCom.length === 0 ? (
          ...
          <div className="space-y-3">
            {filteredCom.map((c) => (
              <ComunicacionCard key={c.id} c={c} onDeleted={onDeleted} />
            ))}
          </div>
        ))}
      ...
      {tab === 'publicaciones' &&
        (filteredPub.length === 0 ? (
          ...
          <div className="space-y-3">
            {filteredPub.map((p) => (
              <PublicacionCard key={p.id} p={p} onDeleted={onDeleted} />
            ))}
          </div>
        ))}
    </div>
...
function ComunicacionCard({ c, onDeleted }: { c: ComunicacionClub; onDeleted: () => void }) {
...
function PublicacionCard({ p, onDeleted }: { p: PublicacionClub; onDeleted: () => void }) {
  const [confirmDelete, setConfirmDelete] = useState(false);
  const [pending, startTransition] = useTransition();
