...
export type PlatformComunicacion = {
  id: string;
  titulo: string;
  texto: string | null;
...
type ModalState = { mode: 'create' } | { mode: 'edit'; comunicacion: PlatformComunicacion } | null;

export function PlatformComunicacionesClient({
  comunicaciones,
}: {
  comunicaciones: PlatformComunicacion[];
}) {
    ...
    <div className="p-4 md:p-8">
      <header className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="page-subtitle mt-1">Anuncios a nivel NauticApp</p>
        </div>
        <button
          type="button"
...
function ComunicacionCard({ c, onEdit }: { c: PlatformComunicacion; onEdit: () => void }) {
...
function ComunicacionModal({
  ...
  const submit = (publicar: boolean) => {
    ...
    const input: PlatformComunicacionInput = {
      titulo: titulo.trim(),
      texto,
      tipo,
      categoria,
    ...
    startTransition(async () => {
      const res = isEdit
        ? await updatePlatformComunicacionAction(state.comunicacion.id, input)
        : await createPlatformComunicacionAction(input);
