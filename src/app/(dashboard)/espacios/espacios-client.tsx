...
import { Anchor, Building2, Plus, Trash2 } from 'lucide-react';
    ...
    <div className="p-8">
      <header className="mb-6 flex flex-wrap items-start justify-between gap-4">
        ...
        <button
          ...
          className="flex items-center gap-2 rounded-[10px] bg-[#175861] px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-60"
        >
      ...
      <section className="mb-6 rounded-2xl border border-gray-200 bg-white p-5">
        ...
        {areas.length === 0 ? (
          ...
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
            {areas.map((a) => (
              <AreaCard key={a.id} area={a} counts={countEspacios(a)} />
            ))}
          </div>
        )}
      </section>
    ...
    <div className="rounded-[14px] border border-gray-200 bg-white p-4">
      <div className="flex items-start justify-between">
        ...
        <button
          ...
          disabled
          title="Disponible en la próxima entrega"
          className="rounded-[8px] p-1 text-red-400 hover:bg-red-50 disabled:opacity-40"
        >
