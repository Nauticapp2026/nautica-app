...
import {
  confirmImportAreasAction,
  previewImportAreasAction,
} from '@/app/actions/bulk-import-areas';
import {
  STATUS_LABEL,
  STATUS_TONE,
  type ImportAreaPreview,
  type PreviewAreasResumen,
} from './bulk-import-areas-types';
...
export function ImportAreasModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  const router = useRouter();
  const fileInputRef = useRef<HTMLInputElement>(null);
  const [step, setStep] = useState<Step>('pick');
  const [file, setFile] = useState<File | null>(null);
  const [preview, setPreview] = useState<ImportAreaPreview[]>([]);
  const [resumen, setResumen] = useState<PreviewAreasResumen | null>(null);
  ...
  function handlePreview() {
    ...
    startTransition(async () => {
      ...
      fd.append('file', file);
      const res = await previewImportAreasAction(fd);
      if (res.error) {
  ...
  function handleConfirm() {
    startTransition(async () => {
      const res = await confirmImportAreasAction({ rows: preview });
      if (res.error) {
        toast.error(res.error);
        return;
      }
      ...
      setStep('done');
      toast.success(`${res.resumen?.creadas ?? 0} áreas creadas.`);
      router.refresh();
    });
  }
        ...
        <div className="flex items-start justify-between border-b border-gray-100 p-6 pb-4">
          <div>
            ...
            <p className="mt-0.5 text-sm" style={{ color: '#669E9D' }}>
              {step === 'pick' &&
                'Cargá masivamente marinas (con peines y amarras) y naves (con lados, pisos y camas).'}
              {step === 'preview' &&
            ...
            <>
              ...
              <div className="flex items-center gap-3">
                <span className="text-sm text-gray-600">
                  Se van a crear <strong>{aProcesar}</strong> área(s) (
                  {resumen?.totalEspaciosACrear ?? 0} espacios)
                </span>
                <button
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 text-sm text-gray-700">
            ...
            <p className="mt-1 text-gray-600">
              Descargá el archivo y completalo. Cada fila es un área entera (Marina o Nave) y el
              sistema autogenera todos los peines/lados, pisos y espacios numerados — igual que el
              botón &quot;Nueva área&quot;.
            </p>
            <a
              href="/templates/areas.xlsx"
              download="plantilla-areas.xlsx"
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        <Stat label="Total filas" value={resumen.total} />
        <Stat label="A crear" value={resumen.aCrear} tone="ok" />
        <Stat label="Con error" value={resumen.conError} tone="error" />
        <Stat label="Espacios totales" value={resumen.totalEspaciosACrear} tone="ok" />
      </div>
            ...
            {rows.map((r) => {
              ...
              return (
                <tr key={r.rowIndex} className="border-t border-gray-100">
                  ...
                  <td className="px-3 py-2 text-gray-700">{r.raw.tipo || '—'}</td>
    ...
    <div className="space-y-5">
      <div className="flex flex-col items-center gap-2 py-4 text-center">
        ...
        <p className="text-sm text-gray-600">
          Las áreas y todos sus espacios quedaron creados. Podés configurar tarifas, ocupantes y
          dimensiones desde el panel.
        </p>
      </div>
