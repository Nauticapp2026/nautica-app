...
import {
  confirmImportSociosAction,
  previewImportSociosAction,
} from '@/app/actions/bulk-import-socios';
import {
  STATUS_LABEL,
  STATUS_TONE,
...
export function ImportSociosModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  function handlePreview() {
    ...
    startTransition(async () => {
      ...
      fd.append('file', file);
      const res = await previewImportSociosAction(fd);
      if (res.error) {
  ...
  function handleConfirm() {
    startTransition(async () => {
      const res = await confirmImportSociosAction({ rows: preview });
      if (res.error) {
        toast.error(res.error);
        return;
      }
      ...
      setStep('done');
      toast.success(`${res.resumen?.creados ?? 0} socios creados.`);
      router.refresh();
    });
  }
  ...
  const aProcesar = (resumen?.aCrear ?? 0) + (resumen?.aVincular ?? 0);
        ...
        <div className="flex items-start justify-between border-b border-gray-100 p-6 pb-4">
          <div>
            ...
            <p className="mt-0.5 text-sm" style={{ color: '#669E9D' }}>
              {step === 'pick' && 'Subí la plantilla completa con los socios a importar.'}
              {step === 'preview' &&
            ...
            <>
              ...
              <div className="flex items-center gap-3">
                <span className="text-sm text-gray-600">
                  Se van a procesar <strong>{aProcesar}</strong> filas
                </span>
                <button
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 text-sm text-gray-700">
            ...
            <p className="mt-1 text-gray-600">
              Descargá el archivo, completalo con los datos de tus socios y volvé a subirlo acá. La
              plantilla tiene una hoja con instrucciones que te explica cada columna.
            </p>
            <a
              href="/templates/socios.xlsx"
              download="plantilla-socios.xlsx"
...
// ─── Preview step ────────────────────────────────────────────────────────────
function PreviewStep({ rows, resumen }: { rows: ImportRowPreview[]; resumen: PreviewResumen }) {
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-3 md:grid-cols-5">
        <Stat label="Total" value={resumen.total} />
        <Stat label="A crear" value={resumen.aCrear} tone="ok" />
        <Stat label="A vincular" value={resumen.aVincular} tone="ok" />
              ...
              return (
                <tr key={r.rowIndex} className="border-t border-gray-100">
                  ...
                  <td className="px-3 py-2 text-gray-900">
                    {[r.raw.nombre, r.raw.apellido].filter(Boolean).join(' ') || '—'}
                  </td>
                  <td className="px-3 py-2 text-gray-700">{r.raw.email || '—'}</td>
    ...
    <div className="space-y-5">
      <div className="flex flex-col items-center gap-2 py-4 text-center">
        ...
        <p className="text-sm text-gray-600">
          Los socios nuevos van a recibir un email de invitación para crear su contraseña.
        </p>
      </div>

      <div className="grid grid-cols-3 gap-3">
        <Stat label="Creados" value={summary.creados} tone="ok" />
        <Stat label="Vinculados" value={summary.vinculados} tone="ok" />
        <Stat label="Saltados" value={summary.saltados} tone="warn" />
      </div>
        ...
        <div className="rounded-xl border border-red-200 bg-red-50 p-4">
          ...
          <ul className="space-y-1 text-xs text-red-700">
            {summary.falladas.map((f) => (
              <li key={`${f.fila}-${f.email}`}>
                Fila {f.fila} ({f.email}): {f.mensaje}
              </li>
            ))}
          </ul>
        </div>
