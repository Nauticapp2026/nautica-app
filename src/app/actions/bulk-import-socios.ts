...
const rowSchema = z.object({
  ...
  direccionNumero: z.string().trim().optional().default(''),
  ciudad: z.string().trim().optional().default(''),
  provincia: z.string().trim().optional().default(''),
  codigoPostal: z.string().trim().optional().default(''),
  contactoEmergencia: z.string().trim().optional().default(''),
  tipoDocumento: z
    .string()
    .trim()
    .optional()
...
export async function previewImportSociosAction(
  ...
  for (let r = 3; r <= ws.rowCount; r++) {
    ...
    const raw: ImportRowRaw = {
      ...
      direccionNumero: readCell(row, 'numero'),
      ciudad: readCell(row, 'ciudad'),
      provincia: readCell(row, 'provincia'),
      codigoPostal: readCell(row, 'codigo_postal'),
      contactoEmergencia: readCell(row, 'contacto_emergencia'),
      ...
      if (row.status === 'nuevo') {
        ...
        await db
          ...
          .values({
            ...
            direccionNumero: row.raw.direccionNumero.trim() || null,
            ciudad: row.raw.ciudad.trim() || null,
            provincia: row.raw.provincia.trim() || null,
            codigoPostal: row.raw.codigoPostal.trim() || null,
            contactoEmergencia: row.raw.contactoEmergencia.trim() || null,
