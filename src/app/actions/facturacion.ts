...
import { enviarReciboPorMail, TIPO_COMPROBANTE_LABEL_MAIL } from '@/lib/email/recibo-mail';
...
export async function crearFacturaCore(
  ...
  const [socio] = await db
    .select({
      ...
      direccionFiscalNumero: profiles.direccionFiscalNumero,
      ciudadFiscal: profiles.ciudadFiscal,
      provinciaFiscal: profiles.provinciaFiscal,
...
/**
 * Envoltorio con sesión sobre `enviarReciboPorMail`. La lógica vive en
 * `lib/email/recibo-mail.ts` porque el cron del débito automático manda el mismo
 * mail y no tiene sesión — y exportar desde un archivo 'use server' una versión
 * que reciba `guarderiaId` por parámetro dejaría pedir recibos de otro club.
 */
export async function enviarReciboPorMailAction(
  ...
  const ctx = await getActiveMarina();
  if (!ctx) return { error: 'No autenticado' };
  return enviarReciboPorMail(reciboId, ctx.activeMembership.guarderiaId);
}
