...
- **Pre-launch gate** — Basic Auth sobre toda la web, en `src/middleware.ts`. **Está apagado desde el 2026-09-16** (se borraron las env vars `PRELAUNCH_GATE_USER` y `PRELAUNCH_GATE_PASSWORD` en Vercel; la web es pública). El código queda: para reactivarlo alcanza con volver a setear las dos vars y redeployar. Rutas excluidas del gate cuando está activo: `/auth/*`, `/api/cron/*`, `/api/webhooks/*`, `/api/devices/*`, `/api/mareas/*`, `/api/delete-account`, `/eliminar-cuenta`, `/privacidad`, `/terminos*`.
- **Buscadores (SEO)** — `src/app/robots.ts` y `src/app/sitemap.ts` generan `/robots.txt` y `/sitemap.xml`; la lista de rutas privadas y el `NOINDEX` viven en `src/lib/seo.ts`. Solo se indexan la landing, `/terminos`, `/privacidad` y `/eliminar-cuenta`. Si agregás una sección nueva a la app (una ruta top-level dentro de `(dashboard)`), sumala a `RUTAS_PRIVADAS`; si agregás un árbol de rutas nuevo fuera de los layouts que ya exportan `NOINDEX`, exportalo desde su layout.
